...
for bldg_num= 1:1
    ...
    %release constraint in the direction of shaking (y)
    ...
    con_nodeset_data= zeros(size(nodeset_data,1),3);
    
    con_nodeset_data(:,1)= [1:size(nodeset_data,1)]';   %node set ID
    con_nodeset_data(:,2)= 4;                           %applicable DOF
    con_nodeset_data(:,3)= 0;                           %TF
    
    fprintf(key_file,'*CONSTRAINED_NODE_SET\n');
    
    for i =1:size(con_nodeset_data,1)
        for ii=1:size(con_nodeset_data,2)
            if ii== size(con_nodeset_data,2)
                fprintf(key_file, '%d', con_nodeset_data(i,ii));
            elseif ii< size(con_nodeset_data,2)
                fprintf(key_file, '%d,', con_nodeset_data(i,ii));
            end
        end
        fprintf(key_file, '\n');
    ...
    fprintf(key_file,'1000,2,1,10000\n');
